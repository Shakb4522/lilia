        ...
        if associated_file_path and not transcribed_text and prompt:
            ...
            if needs_transcribe:
                try:
                    ...
                    if DEEPGRAM_API_KEY:
                        tg_url = "https://api.deepgram.com/v1/listen?model=nova-3&smart_format=true&detect_language=true"
                        headers = {"Authorization": f"Token {DEEPGRAM_API_KEY}"}
...
async def websocket_endpoint(websocket: WebSocket, lang: str = "en"):
    ...
    deepgram_url = f"wss://api.deepgram.com/v1/listen?model=nova-3&smart_format=true&interim_results=true&language={lang}"
    headers = {"Authorization": f"Token {DEEPGRAM_API_KEY}"}
